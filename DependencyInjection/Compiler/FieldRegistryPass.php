...
class FieldRegistryPass implements CompilerPassInterface
{
    ...
    {
        if ( !$container->hasDefinition( 'ezpublish.persistence.solrdoc.search.field_registry' ) )
        {
            return;
        }

        $fieldRegistryDefinition = $container->getDefinition( 'ezpublish.persistence.solrdoc.search.field_registry' );
