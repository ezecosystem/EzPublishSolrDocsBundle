...
class AggregateSortClauseVisitorPass implements CompilerPassInterface
{
    ...
    {
        if ( !$container->hasDefinition( 'ezpublish.persistence.solrdoc.search.content.sort_clause_visitor.aggregate' ) )
        {
            return;
        }
        ...
        foreach ( $container->findTaggedServiceIds( 'ezpublish.persistence.solrdoc.search.content.sort_clause_visitor' ) as $id => $attributes )
        {
