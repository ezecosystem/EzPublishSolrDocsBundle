...
class SignalSlotPass implements CompilerPassInterface
{
    public function process( ContainerBuilder $container )
    {
        ...
        foreach ( $container->findTaggedServiceIds( 'ezpublish.persistence.solrdoc.slot' ) as $id => $attributes )
        {
            foreach ( $attributes as $attribute )
            {
                if ( !isset( $attribute['signal'] ) )
                {
                    throw new LogicException(
                        ...
                        "which is mandatory for services tagged as 'ezpublish.persistence.solrdoc.slot'"
                    );
                }
